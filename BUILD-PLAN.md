...
> The marketing + dashboard experience redesign is complete (item 43); the next unshipped product priority remains T2-4.
...
42. ~~**Pricing page.**~~ ✅ Done — `feat/pricing-page-v2`. Closes the Roadmap "Pricing page (Managed SaaS)" item. New `/pricing` route: hero, self-host row, Annual/Monthly plan toggle (`PricingToggle`, 20% annual discount via `annualMonthlyPrice()` in `lib/billing/plans.ts` — display-only, no separate Stripe annual price exists yet), 15-row × 4-tier feature comparison table (`PricingComparisonTable`), FAQ, waitlist CTA. Nav/footer "Pricing" links across the marketing chrome now point at `/pricing` instead of the old `/#pricing` anchor. In-app billing settings (`app/(dashboard)/billing/page.tsx`) gained a `LimitWarningBanner` that surfaces once usage crosses 80% of the plan's deflection limit, with a one-click upgrade to the next tier. License and framing cleanup requested during review: dropped every "AGPL-3.0" badge/mention from marketing surfaces (landing page, `/pricing`, nav, footer) — license details now live on GitHub only, not repeated across product pages — and replaced "free forever" self-host framing with "try it free for 14 days" for hosted plans, since that's the actual funnel or self-hosting for users who want their own infra. New: `app/pricing/page.tsx`, `components/marketing/pricing-toggle.tsx`, `components/marketing/pricing-comparison-table.tsx`. `/pricing` added to `auth.ts`'s `PUBLIC_PATHS` (unauthenticated visitors were 307'd to `/login` otherwise — same bug class `/vs` hit). Docs: `docs/product/billing.mdx`, `docs/reference/billing-plans.mdx` filled in from stubs. Tests: `tests/unit/pricing-toggle.test.tsx`, `tests/unit/billing-limit-warning.test.ts`, extended `tests/unit/auth-public-paths.test.ts`.
43. ~~**Marketing + dashboard experience redesign.**~~ ✅ Done — `codex/redesign-marketing-dashboard`. Reworks the public landing page into a scroll-driven blue/black product story with a realistic animated support workflow, clearer proof sections, higher-contrast copy, and a rebuilt pricing presentation. Replaces the dashboard’s generic admin-template shell with a grouped command-center navigation and a consistent visual system across Dashboard, Analytics, Tickets, ticket detail, Knowledge Base, FAQ, Knowledge Gaps, Leads, Simulation, Billing, and Settings. Revoked API keys now disappear from the active credential list immediately and stay hidden after refresh while their database records remain available for audit history. **Files:** `app/page.tsx`, `app/pricing/page.tsx`, `app/(dashboard)/**/*.tsx`, `app/globals.css`, `components/animated-chat.tsx`, `components/marketing/*.tsx`, `components/sidebar-nav.tsx`, `components/dashboard/stats-cards.tsx`, `components/tickets/ticket-list.tsx`, `lib/db/queries/api-keys.ts`, `docs/product/dashboard.mdx`, `docs/integrations/mcp.mdx`. **Tests:** `tests/unit/animated-chat.test.tsx`, `tests/unit/dashboard-navigation.test.tsx`, `tests/unit/pricing-toggle.test.tsx`, `tests/unit/mcp-server.test.ts`.
