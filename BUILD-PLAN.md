...
38. **GEO (Generative Engine Optimization) — Tier 1 shipped** — 🚧 In progress, `feat/geo-tier1`. Goal: get ChatGPT/Perplexity/Claude to recommend AnswerLoops in conversation (separate from the MCP server, which is the "AI agents *use* AnswerLoops as a tool" half — already shipped). Tier 1 code deliverables done: `public/llms.txt` (plain-English product description for AI crawlers, per the Jeremy Howard/fast.ai proposed standard), `public/.well-known/ai-plugin.json` + `public/openapi.json` (ChatGPT plugin/action discovery manifest — honestly scoped: the OpenAPI doc only covers the real `/api/health` REST endpoint and points agents at the actual MCP JSON-RPC surface for tool calls, since no `/api/agent/*` REST layer exists yet), and Schema.org `SoftwareApplication` + `Offer` JSON-LD on the landing page (`app/page.tsx`) built from `ORDERED_PLANS` so AI search engines can extract accurate pricing instead of parsing prose. **Deliberately not done as code:** `FAQPage` JSON-LD (skipped — the landing page has no visible FAQ section yet, and Google's guidelines require structured data to mirror visible content; revisit once Tier 2's "GEO content" FAQ section ships). **Not code — manual submission tasks, tracked separately:** MCP registry listings (mcp.so, Smithery.ai, Glama.ai, Awesome-MCP-Servers), product directories (G2, Capterra, Product Hunt, Futurepedia, There's An AI For That). Tier 2 (GPT Store Action, comparison pages, GEO-phrased FAQ content) and Tier 3 (Show HN, Reddit, dev.to, Awesome lists) remain open — see Roadmap page "AI surfacing / GEO" section.
