...
GitHub and Notion syncs run as background jobs — the Knowledge Base page shows each one as queued, then syncing, then done, and you can navigate away while it runs. Re-triggering a sync for the same source while one is still in progress does nothing; the running job finishes on its own.
