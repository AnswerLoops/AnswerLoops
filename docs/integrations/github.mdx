---
title: GitHub
description: Ingest GitHub Issues and Discussions as tickets, and sync repo markdown into the Knowledge Base.
---

## Overview

The GitHub integration does two things:
...
2. **KB sync** — markdown files in a repo are embedded into the Knowledge Base. Syncs automatically on every push (when enabled) or manually via **Sync now**.
...
- **Monitored events** — choose Issues, Discussions, Both, or None
- **Knowledge Base source** — toggle KB sync on/off; **Sync now** triggers an immediate sync

When KB sync is enabled, every push to the default branch re-embeds all `.md` and `.mdx` files automatically.
