...
- Rate limited per organization (shared across all of that org's keys): 60 requests/minute
...
`question` is capped at 2000 characters. Returns `{ answer, confidence, answered_fully, high_confidence }`. This counts against the organization's monthly deflection limit — if the limit is reached, the tool returns an error instead of generating for free, matching the same metering every other channel is subject to.
