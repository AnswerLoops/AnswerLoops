...
## The two concerns — and how we address them
...
See [Docker Setup](/self-hosting/docker) and [Production Checklist](/self-hosting/production).
...
- [ ] **No admin OAuth scopes** — only `channels:history`, `channels:read`, `chat:write`, `reactions:write`
- [ ] **Outbound-only** — bot container initiates all connections; no inbound HTTP
- [ ] **Encrypted secrets** — bot tokens and API keys stored with AES-256-GCM encryption at rest
- [ ] **Self-hostable** — all data stays in your infrastructure
- [ ] **No message content leaves your servers** — AI calls use your own API key, go direct to your AI provider
- [ ] **Configurable poll interval** — `SLACK_POLL_INTERVAL_SECONDS` (default 60s)
- [ ] **Cursor-based dedup** — no message processed twice; safe to run across restarts
