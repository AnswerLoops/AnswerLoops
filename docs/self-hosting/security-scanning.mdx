...
<Steps>
  <Step title="Keep your deployment up to date">
    Security fixes ship as regular releases. See [Upgrading](/self-hosting/upgrading) — pull the latest image or `git pull` + rebuild regularly rather than pinning to an old version indefinitely.
  </Step>
  <Step title="Set a real ENCRYPTION_KEY">
    AI provider API keys and bot tokens are encrypted at rest using AES-256-GCM, keyed by `ENCRYPTION_KEY`. Without it set, secrets are stored in plaintext — see [Environment Variables](/self-hosting/environment-variables). Generate one with `openssl rand -hex 32` and never commit it to source control.
  </Step>
